...
| **Code Reviewer** | `reviewer`     | Reviews code changes with severity ratings               |
...
Delegate to **Code Reviewer** (`reviewer`) with:

- List of files changed
- Project standards to check against (existing patterns, conventions)

#### 5c. Analyze Results (CRITICAL — DO NOT SKIP)
...
- 🔴 High issues found → delegate a fix to **Implementor** or **Debugger** with the EXACT issues quoted from the reviewer (file path, problem description, suggested fix). Then re-run 5a–5c.
